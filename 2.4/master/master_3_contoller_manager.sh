...
check_1_3_7="1.3.7  - Ensure that the --bind-address argument is set to 127.0.0.1"
address3=$(docker inspect kube-controller-manager | jq -e '.[0].Args[] | match("--bind-address=127\\.0\\.0\\.1").string')
if [ "$address3" = \"--bind-address=127.0.0.1\" ]; then
  	pass "$check_1_3_7"
else
  	warn "$check_1_3_7"
fi
