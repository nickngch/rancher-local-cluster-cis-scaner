...
certs=(`ls -laR /etc/kubernetes/ssl/ |grep -v yaml | grep -v key | grep -v ^d | grep -v ^/ | grep -v ^t | grep pem | cut -d " " -f11`)
info "$check_1_1_20"
...
keys=(`ls -laR /etc/kubernetes/ssl/ |grep -v yaml |  grep -v ^d | grep -v ^/ | grep -v ^t | grep key | cut -d " " -f11`)
info "$check_1_1_21"
  for ((i=0; i<${#keys[@]}; i++)); do
